...
//=================================================================================================================
//           [These sections are for folding control  in Code::Blocks]         [$Date: 2021-09-14 18:45:19 +0400 $]
...
#define _TX_VER      _TX_v_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 171, 2021-09-14 18:45:19 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
#define _TX_VERSION  _TX_V_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 171, 2021-09-14 18:45:19 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
#define _TX_AUTHOR   _TX_A_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 171, 2021-09-14 18:45:19 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
...
DECLARE_INTERFACE_ (ITaskbarList, IUnknown)
    ...
    STDMETHOD (AddTab)                (THIS_ HWND wnd)                                         PURE;
    STDMETHOD (DeleteTab)             (THIS_ HWND wnd)                                         PURE;
    STDMETHOD (ActivateTab)           (THIS_ HWND wnd)                                         PURE;
    STDMETHOD (SetActiveAlt)          (THIS_ HWND wnd)                                         PURE;
    };
...
DECLARE_INTERFACE_ (ITaskbarList2, ITaskbarList)
    {
    STDMETHOD (MarkFullscreenWindow)  (THIS_ HWND wnd, BOOL fullscreen)                        PURE;
    };
...
DECLARE_INTERFACE_ (ITaskbarList3, ITaskbarList2)
    {
    STDMETHOD (SetProgressValue)      (THIS_ HWND wnd, ULONGLONG completed, ULONGLONG total)   PURE;
    STDMETHOD (SetProgressState)      (THIS_ HWND wnd, unsigned flags)                         PURE;
    STDMETHOD (RegisterTab)           (THIS_ HWND wndTab, HWND wndMDI)                         PURE;
    STDMETHOD (UnregisterTab)         (THIS_ HWND wndTab)                                      PURE;
    STDMETHOD (SetTabOrder)           (THIS_ HWND wndTab, HWND wndInsertBefore)                PURE;
    STDMETHOD (SetTabActive)          (THIS_ HWND wndTab, HWND wndMDI, DWORD reserved)         PURE;
    STDMETHOD (ThumbBarAddButtons)    (THIS_ HWND wnd, UINT buttons, const void* button)       PURE;
    STDMETHOD (ThumbBarUpdateButtons) (THIS_ HWND wnd, UINT buttons, const void* button)       PURE;
    STDMETHOD (ThumbBarSetImageList)  (THIS_ HWND wnd, HIMAGELIST himl)                        PURE;
    STDMETHOD (SetOverlayIcon)        (THIS_ HWND wnd, HICON icon, const wchar_t* description) PURE;
    STDMETHOD (SetThumbnailTooltip)   (THIS_ THIS_ HWND wnd, const wchar_t* tip)               PURE;
    STDMETHOD (SetThumbnailClip)      (THIS_ HWND wnd, RECT* clip)                             PURE;
    };
...
#define IShellLinkDataList            Win32::IShellLinkDataList
#define ITaskbarList                  Win32::ITaskbarList
#define ITaskbarList2                 Win32::ITaskbarList2
#define ITaskbarList3                 Win32::ITaskbarList3
#define TBPFLAG                       Win32::TBPFLAG
...
#define _TX_DLLIMPORT_CRT( lib, retval, name, params )  TX_DLLIMPORT (false, lib ".dll", retval, name, params)
...
$   if (mangledName)
        ...
        if (_txSetJmp())
            ...
            if (false) ;
            PRINT_VAL_ ("%s", char*)   PRINT_VAL_ ('%c', unsigned char)    PRINT_VAL_ (%s,   bool, ? "true" : "false")  //-V206 //-V517
            PRINT_VAL_ ( %d,  int)     PRINT_VAL_ ( %u,  unsigned int)     PRINT_VAL_ (%g,   float)                     //-V206
            PRINT_VAL_ ( %hd, short)   PRINT_VAL_ ( %hu, unsigned short)   PRINT_VAL_ (%lg,  double)                    //-V206
            PRINT_VAL_ ( %ld, long)    PRINT_VAL_ ( %lu, unsigned long)    PRINT_VAL_ ('%c', char)                      //-V206
...
HDC txCreateDIBSection (double sizeX, double sizeY, RGBQUAD** pixels /*= NULL*/)
    {
$1  RGBQUAD* pix = NULL;
    if (!pixels) {$ pixels = &pix; }

$   HDC dc = txCreateCompatibleDC (sizeX, sizeY, NULL, pixels);

$   RGBQUAD black = { 0, 0, 0, 255 };
$   for (int i = 0; i < sizeX * sizeY; i++) (*pixels)[i] = black;  //-V108
